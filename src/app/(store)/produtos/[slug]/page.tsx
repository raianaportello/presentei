...
export default async function ProductPage({ params }: PageProps<"/produtos/[slug]">) {
  ...
  const jsonLd = { "@context": "https://schema.org", "@type": "Product", name: product.name, description: product.description, image: product.images.map((image) => image.url), brand: { "@type": "Brand", name: "Presentei" }, offers: { "@type": "Offer", priceCurrency: "BRL", price: "39.90", availability: "https://schema.org/InStock", url: `${process.env.NEXT_PUBLIC_SITE_URL ?? "http://localhost:3000"}/produtos/${slug}` } };
